...
$fail_count=array();
$success_up=0;
foreach($xml->HDBills->HDBill as $tesss){
	foreach($tesss->TBill as $te){
	  	$hn = $te->hn;
	  	$invno = $te->invno;
	  	$vstdate = $te->dttran; # vstdate
	  	$stm_claim = $te->amount; # stm_claim
	  	$stm_rep = $tesss->wkno; # stm_rep
	  	#echo $hn." ".$invno." ".$vstdate." ".$stm_claim." ".$stm_rep;
	  	#echo "<BR>";

		$i++;
		//สำหรับเช็ค ว่าอัพโหลดหรือยัง และ สำหรับจับกับ Visit
		$vstdate_before=(str_replace("T","",str_replace("-","",str_replace(":","",$vstdate))));
		$sub_year=substr($vstdate_before, 0, 4);
		$sub_month=substr($vstdate_before, 4, 2);
		$sub_day=substr($vstdate_before, 6, 2);
		$sub_time=substr($vstdate_before, 8, 4);
		$time=$sub_time."00";

		$match_stm=$hn.$sub_day.$sub_month.$sub_year.$time;
		...
		if( isset($check_rep_r_s_f) ){
			...
			$strSQL = "
				INSERT INTO stm 
				(match_stm, stm_rep, stm_claim, stm_file_name, stm_user_reccord, stm_hn
				,stm_type) 
				VALUES ('".$match_stm."','".$stm_rep."','".$stm_claim."','".$file_name."','".$stm_user_reccord."','".$hn."'
				,'stm_sss_tai'
				) ";
