...
return new class extends Migration
{
    ...
    { 
        if (!Schema::hasTable('wh_stock_dep'))
        {
            Schema::create('wh_stock_dep', function (Blueprint $table) {
                $table->bigIncrements('wh_stock_dep_id');  
                $table->string('wh_request_id')->nullable(); // 
                $table->string('year')->nullable();               //  ปีงบประมาณ
                $table->date('stock_date')->nullable();         //  วันที่รับ 
                $table->time('stock_time')->nullable();         //  เวลา
    ...
    {
        Schema::dropIfExists('wh_stock_dep');
    }
};
