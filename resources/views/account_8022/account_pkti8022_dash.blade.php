...
<div class="tabs-animation">
    ...
    <div id="preloader">
        <div id="status">
            ...
            <form action="{{ URL('account_pkti8022_dash') }}" method="GET">
                @csrf
                <div class="row">
                    <div class="col"></div>
                    <div class="col-md-7">
                        <h4 class="card-title">Detail 1102050101.8022</h4>
                        <p class="card-title-desc">รายละเอียดข้อมูล ผัง 1102050101.8022</p>
                    </div>
                    {{-- <div class="col"></div> --}}
                                ...
                                <table id="example" class="table table-striped table-bordered dt-responsive nowrap myTable"
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center" style="background-color: rgb(253, 135, 174)">ต้องตั้ง-8022</th>
                                            <th class="text-center" style="background-color: rgb(253, 135, 174)">ตั้งลูกหนี้-8022</th>
                                            <th class="text-center" style="background-color: rgb(253, 135, 174)">Stm-8022</th>
                                            ...
                                            <?php
                                                ...
                                                $datas = DB::select('
                                                    SELECT count(DISTINCT an) as Can
                                                        ...
                                                        from acc_debtor
                                                        WHERE account_code="1102050101.8022"
                                                ...
                                                foreach ($datasum_ as $key => $value2) {
                                                 ...
                                                 $datas4022 = DB::select('
                                                    SELECT count(DISTINCT an) as Can
                                                        ,SUM(debit_total) as sumdebit
                                                        from acc_debtor
                                                        WHERE account_code="1102050102.8022"
                                                        AND stamp = "N" AND debit_total > 0
                                                        AND month(dchdate) = "'.$item->months.'"
                                                        AND year(dchdate) = "'.$item->year.'"
                                                ');
                                                ...
                                                // STM 402
                                                $stm_ = DB::select(
                                                    'SELECT sum(stm_money) as stm_money,count(DISTINCT an) as Countvisit FROM acc_1102050102_8022
                                                ...
                                                foreach ($stm_8022 as $key => $value4) {
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end" style="color:rgb(45, 57, 230);background-color: rgb(255, 174, 201)" width="10%">
                                                       <a href="{{url('account_pkti8022_pull')}}" target="_blank" style="color:rgb(21, 85, 223);"> {{ number_format($sum_N4022, 2) }}</a>
                                                    </td>
                                                    <td class="text-end" width="10%" style="background-color: rgb(255, 174, 201)">
                                                        <a href="{{url('account_pkti8022_detail/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(101, 12, 153);"> {{ number_format($sum_fokliad, 2) }}</a>
                                                    </td>
                                                    <td class="text-end" style="color:rgb(5, 114, 96);background-color: rgb(255, 174, 201)" width="10%">
                                                        <a href="{{url('account_pkti8022_stm/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(101, 12, 153);">{{ number_format($sum_stm_moneyti, 2) }}</a>
                                                    </td>
