        ...
        <div id="status">
            ...
            <form action="{{ route('acc.account_602_dash') }}" method="GET">
                ...
                <div class="row">
                    <div class="col"></div>
                    <div class="col-md-7">
                        <h4 class="card-title" style="color:rgb(247, 31, 95)">Detail 1102050102.602</h4>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($datashow as $item)
                                            <?php
                                                ...
                                                //     FROM acc_debtor
                                                //     WHERE month(vstdate) = "'.$item->months.'" AND account_code ="1102050102.602"
                                                //     AND year(vstdate) = "'.$item->year.'"
                                                //     AND active_claim = "Y"
                                                // ');
                                                ...
                                                // ตั้งลูกหนี้ OPD
                                                $datasum_ = DB::select(
                                                    // 'SELECT sum(nhso_ownright_pid) as debit_total,count(DISTINCT vn) as Cvit
                                                ...
                                                // STM
                                                $stm_ = DB::select(
                                                    'SELECT count(DISTINCT U1.vn) as Countvisit ,sum(U1.nhso_ownright_pid) as stm_money
                                                        FROM acc_1102050102_602 U1
                                                        WHERE month(U1.vstdate) = "'.$item->months.'"
                                                        AND year(U1.vstdate) = "'.$item->year.'"
                                                        AND U1.nhso_ownright_pid >= "0.00"
                                                ');
                                                ...
                                                // ยกไป
                                                $yokpai_ = DB::select(
                                                    'SELECT sum(debit_total) as debit_yok,count(vn) as Countvi
                                                        FROM acc_1102050102_602
                                                        WHERE month(vstdate) = "'.$item->months.'"
                                                        AND year(vstdate) = "'.$item->year.'"
                                                        AND (nhso_ownright_pid IS NULL OR nhso_ownright_pid = "")
                                                ');
                                                foreach ($yokpai_ as $key => $valpai) {
                                                    $sum_yokpai = $valpai->debit_yok;
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end" style="color:rgb(6, 82, 170);background-color: rgb(203, 227, 255)" width="10%">
                                                        <a href="{{url('account_602_pull')}}" target="_blank" style="color:rgb(5, 58, 173);"> {{ number_format($sum_N, 2) }}</a>
                                                    </td>
                                                    <td class="text-end" style="color:rgb(231, 73, 139);background-color: rgb(203, 227, 255)" width="10%">
                                                        <a href="{{url('account_602_detail/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(231, 73, 139);"> {{ number_format($total_sumY, 2) }}</a>
                                                    </td>

                                                    <td class="text-end" style="color:rgb(2, 116, 63);background-color: rgb(203, 227, 255)" width="10%">
                                                         {{-- <a href="{{url('account_602_stm/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(2, 116, 63);"> --}}
                                                            {{ number_format($sum_stm_money, 2) }}
                                                        {{-- </a> --}}
                                                    </td>

                                                    <td class="text-end" style="color:rgb(224, 128, 17)" width="10%">
                                                        {{-- <a href="{{url('account_602_stmnull/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(224, 128, 17);"> --}}
