...
<div class="tabs-animation">
    ...
    <div id="preloader">
        <div id="status">
            ...
            <form action="{{ route('acc.account_pkti8011_dash') }}" method="GET">
                @csrf
                <div class="row"> 
                    <div class="col"></div>
                    <div class="col-md-7">
                        <h4 class="card-title" style="color:green">Detail 1102050101.8011</h4>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($datashow as $item)
                                            ...
                                            //     }
                                                ...
                                                // STM 8011
                                                $stm_ = DB::select(
                                                    'SELECT count(DISTINCT U1.vn) as Countvisit ,sum(U2.pay_amount) as stm_total
                                                ...
                                                // ยกไป
                                                $yokpai_ = DB::select(
                                                    'SELECT sum(U1.debit_total) as debit_total,count(U1.vn) as Countvi
                                                        FROM acc_1102050102_8011 U1                                                          
                                                        LEFT JOIN acc_stm_lgoti U2 on U2.cid = U1.cid AND U2.vstdate = U1.vstdate
                                                        WHERE month(U1.vstdate) = "'.$item->months.'"
                                                        AND year(U1.vstdate) = "'.$item->year.'"
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end" style="color:rgb(6, 82, 170);background-color: rgb(203, 227, 255)" width="10%">
                                                        <a href="{{url('account_pkti8011_pull')}}" target="_blank" style="color:rgb(5, 58, 173);"> {{ number_format($sum_N, 2) }}</a>
                                                    </td>                                                    
                                                    <td class="text-end" style="color:rgb(231, 73, 139);background-color: rgb(203, 227, 255)" width="10%"> 
                                                        <a href="{{url('account_pkti8011_detail/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(231, 73, 139);"> {{ number_format($total_sumY, 2) }}</a> 
                                                    </td> 
                                                    <td class="text-end" style="color:rgb(2, 116, 63);background-color: rgb(203, 227, 255)" width="10%"> 
                                                         <a href="{{url('account_pkti8011_stm/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(2, 116, 63);"> {{ number_format($sum_stm_money, 2) }}</a> 
                                                    </td> 
                                                  
                                                    <td class="text-end" style="color:rgb(224, 128, 17)" width="10%">  
                                                        <a href="{{url('account_pkti8011_yok/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(224, 128, 17);"> {{ number_format($sum_yokpai, 2) }}</a> 
                                                    </td>
                                                </tr>
                                            <?php
