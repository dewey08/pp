                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($datashow as $item)
                                            <?php
                                                ...
                                                // ตั้งลูกหนี้ IPD 217
                                                $datasum_ = DB::select('
                                                    SELECT SUM(debit_total) as debit_looktotal,count(DISTINCT an) as Cvit
                                                    from acc_1102050101_217
                                                    where month(dchdate) = "'.$item->months.'"
                                                    AND year(dchdate) = "'.$item->years.'"

                                                ');
                                                // SELECT SUM(DISTINCT debit_total) as debit_looktotal,count(DISTINCT an) as Cvit
                                                // WHERE month(dchdate) = "'.$months.'" and year(dchdate) = "'.$year.'"
                                                foreach ($datasum_ as $key => $value2) {
                                                    $total_sumlookY = $value2->debit_looktotal;
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-end" style="color:rgb(231, 73, 139);background-color: rgb(203, 227, 255)" width="10%">
                                                        <a href="{{url('account_pkucs217_detail/'.$item->months.'/'.$item->years)}}" target="_blank" style="color:rgb(231, 73, 139);"> {{ number_format($total_sumlookY, 2) }}</a>
                                                    </td>
