    ...
    <div class="tabs-animation">
        ...
        <form action="{{ route('acc.account_pkucs209_dash') }}" method="GET">
            ...
            <div class="row">
                <div class="col"></div>
                <div class="col-md-5">
                    <h4 class="card-title" style="color:rgb(247, 31, 95)">Detail Data Pang 4301020105.228</h4>
                    <p class="card-title-desc">รายละเอียดข้อมูล ผัง บริการฟื้นฟูสมรรถภาพด้านการแพทย์</p>
                </div>
                {{-- <div class="col"></div> --}}

                @if ($budget_year =='')
                <div class="col-md-2">
                        <select name="budget_year" id="budget_year" class="form-control-sm inputmedsalt text-center" style="width: 100%">
                                    ...
                                    @foreach ($datashow as $item)
                                        <?php
                                            ...
                                            $datas = DB::select('
                                                SELECT count(DISTINCT vn) as Can
                                                    ...
                                                    from acc_debtor
                                                    WHERE account_code="4301020105.228"
                                            ...
                                            // STM
                                            $stm_ = DB::select(
                                                'SELECT sum(debit_total) as debit_total,sum(stm_chodchey) as stm_chodchey,count(DISTINCT vn) as Countvisit FROM acc_4301020105_228
                                                WHERE month(vstdate) = "'.$item->months.'" AND year(vstdate) = "'.$item->year.'" AND (stm_chodchey IS NOT NULL OR stm_chodchey <> "")
                                            ');
                                            foreach ($stm_ as $key => $value3) {
                                                $sum_debit_total  = $value3->debit_total;
                                                $sum_stm_money  = $value3->stm_chodchey;
                                                $count_stm      = $value3->Countvisit;
                                            }

                                            // ยกไป
                                            $yokpai_ = DB::select(
                                                'SELECT sum(debit_total) as debit_total,count(DISTINCT vn) as Countvi FROM acc_4301020105_228
                                                WHERE month(vstdate) = "'.$item->months.'" AND year(vstdate) = "'.$item->year.'" AND (stm_chodchey IS NULL OR stm_chodchey = "")
                                                ');
                                        ...
                                        ?>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end" width="10%"><a href="{{url('kontoon_imc_detail/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(186, 75, 250)"> {{ number_format($sum_Y, 2) }}</a></td>
                                                <td class="text-end" style="color:rgb(4, 161, 135)" width="10%">
                                                    {{-- <a href="{{url('kontoon_imc_stm/'.$item->months.'/'.$item->year)}}" target="_blank" style="color:rgb(4, 161, 135)"> {{ number_format($sum_stm_money, 2) }}</a> --}}
