...
class AccountController extends Controller
{
        ...
        if ($startdate == '') {
            ...
            $datashow = DB::connection('mysql10')->select(
                'SELECT YEAR(v.vstdate) as year,MONTH(v.vstdate) as months,l.MONTH_NAME,COUNT(DISTINCT v.vn) AS count_vn,SUM(v.income) AS sum_income
                    ,SUM(v.paid_money) AS sum_paid_money,SUM(v.rcpt_money) AS sum_rcpt_money,SUM(v.uc_money) AS sum_uc_money
    ...
    public function account_nopaid_sub(Request $request,$startdate,$enddate)
    {
        // $startdate   = $request->startdate;
        // $enddate     = $request->enddate;
        // $years       = $request->years;
        // $monthss      = $request->monthss;
        ...
        // if ($startdate != '') {
            ...
            // WHERE YEAR(v.vstdate) = "' . $years . '" AND MONTH(v.vstdate) = "' . $monthss . '"
        // } else {
        //     $datashow = DB::connection('mysql10')->select('
        //         SELECT v.vn, v.income,v.uc_money,p.cid, v.hn, v.vstdate,o.vsttime,o.main_dep,k.department, v.pdx, v.pttype,concat(p.pname,p.fname," ",p.lname) ptname,o.staff
        //         ,r.bill_date_time,r.finance_number,r.rcpno,r.bill_amount,r.user,r.book_number,r.total_amount,v.paid_money,v.rcpt_money,v.remain_money
