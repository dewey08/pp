...
class AccountController extends Controller
{
    ...
    public function account_nopaid(Request $request)
    {
        ...
        if ($startdate == '') {
            ...
            $months_old  = ('10');
            $datashow = DB::connection('mysql10')->select(
                'SELECT YEAR(v.vstdate) as year,MONTH(v.vstdate) as months,l.MONTH_NAME,COUNT(DISTINCT v.vn) AS count_vn
                ,SUM(v.income) AS sum_income
                    ,SUM(v.uc_money) AS sum_uc_money
                    ,SUM(v.paid_money) AS sum_paid_money
                    ,SUM(v.remain_money) AS sum_remain_money
            ...
            $datashow = DB::connection('mysql2')->select(
                'SELECT v.vstdate,YEAR(v.vstdate) as year
                    ...
                    AND v.pttype NOT IN("M1","M2","M3","M4","M5","M6","P1")
                    AND v.rcpt_money <> 30
                    GROUP BY date_format(v.vstdate, "%D")
                    ORDER BY v.vstdate desc
            ');
    ...
    public function account_nopaid_sub(Request $request,$vstdate)
    {
            $datashow = DB::connection('mysql10')->select('
                SELECT v.vn, v.income,v.uc_money,p.cid, v.hn, v.vstdate,o.vsttime,o.main_dep,k.department, v.pdx, v.pttype,concat(p.pname,p.fname," ",p.lname) ptname,o.staff
    ...
    public function account_nopaid_ip(Request $request)
    {
        ...
        if ($startdate != '') {
            $datashow = DB::connection('mysql10')->select(
                'SELECT a.dchdate,YEAR(a.dchdate) as year,MONTH(a.dchdate) as months
                    ,SUM(a.income) AS sum_income,SUM(a.paid_money) AS sum_paid_money,SUM(a.rcpt_money) AS sum_rcpt_money
                    ,SUM(a.uc_money) AS sum_uc_money
                    ,SUM(a.remain_money) AS sum_remain_money
                    ,COUNT(DISTINCT a.an) AS count_an,l.MONTH_NAME,SUM(a.paid_money)-SUM(a.rcpt_money) as sum_Total
                FROM an_stat a
                LEFT JOIN pttype t on t.pttype=a.pttype
                LEFT JOIN leave_month l on l.MONTH_ID = MONTH(a.dchdate)
                ...
                left outer join rcpt_print r on r.vn = i.vn
                    WHERE a.dchdate BETWEEN "' . $startdate . '" and "' . $enddate . '"
                    AND (a.paid_money > 0 and a.rcpt_money = "0.000" and a.remain_money = "0.000")
                    GROUP BY date_format(a.dchdate, "%D")
                    ORDER BY a.dchdate desc
            ');
    ...
    public function account_nopaid_sub_ip(Request $request,$dchdate)
    {
        $startdate = $request->startdate;
        $enddate = $request->enddate;

        $datashow = DB::connection('mysql10')->select(
            'SELECT a.an,a.hn,p.cid,a.dchdate,concat(p.pname,p.fname," ",p.lname) as ptname,k.department,rp.book_number,rp.total_amount,a.uc_money
