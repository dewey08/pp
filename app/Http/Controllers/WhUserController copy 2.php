        ...
        if ($startdate =='') {
            $data['wh_stock_sub']         = DB::select(
                'SELECT e.wh_stock_sub_id,e.stock_list_subid,w.wh_request_id,w.request_date,f.DEPARTMENT_SUB_SUB_NAME,a.pro_year,a.pro_id,a.pro_code,a.pro_name,b.wh_type_name,c.wh_unit_name,g.one_price
                    ...
                    WHERE s.pro_id = e.pro_id AND s.stock_sub_year ="'.$bg_yearnow.'" AND s.stock_list_subid ="'.$dep_subsubtrueid.'" AND sm.active ="REPEXPORT" AND sm.request_date BETWEEN "'.$newDate.'" AND "'.$datenow.'") AS request_qty
                    ,(SELECT SUM(s.qty_pay) FROM wh_stock_dep_sub s LEFT JOIN wh_request sm ON sm.wh_request_id = s.wh_request_id
                    WHERE s.pro_id = e.pro_id AND s.stock_sub_year ="'.$bg_yearnow.'" AND s.stock_list_subid ="'.$dep_subsubtrueid.'" AND sm.active ="REPEXPORT" AND sm.request_date BETWEEN "'.$newDate.'" AND "'.$datenow.'") AS request_qty_pay
                    ...
                    WHERE s.pro_id = e.pro_id AND s.stock_sub_year ="'.$bg_yearnow.'" AND s.stock_list_subid ="'.$dep_subsubtrueid.'" AND sm.active ="REPEXPORT" AND sm.request_date BETWEEN "'.$newDate.'" AND "'.$datenow.'") AS request_total_price
                    ...
                    LEFT JOIN department_sub_sub f ON f.DEPARTMENT_SUB_SUB_ID = e.stock_list_subid
                    WHERE a.active ="Y" AND w.request_date BETWEEN "'.$newDate.'" AND "'.$datenow.'"
    ...
    public function wh_subpay_select_lotsave(Request $request)
    {
        ...
        if ($count > 0) {
            ...
            Wh_stock_dep_sub_export::insert([
                ...
                'qty_pay_sub'             => $qty_pay_sub,
                'total_stock'             => $count_stock->qty_pay-$qty_pay_sub,
                // 'total_stock'             => $qty_stock-$qty_pay_sub,
                'unit_id'                 => $unitid,
                'unit_name'               => $unitname,
                'one_price'               => $one_price,
                'total_price'             => $qty_pay_sub*$one_price,
