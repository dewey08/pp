...
class Account307Controller extends Controller
 {
    ...
    public function account_307_pull(Request $request)
    {
        ...
        if ($startdate == '') {
            ...
            $acc_debtor = DB::select('
                ...
                WHERE a.account_code="1102050101.307" AND a.vstdate BETWEEN "'.$new_day.'" AND  "'.$date.'"
                order by a.vstdate desc;
            ');
            ...
            $acc_debtor = DB::select('
                SELECT a.* from acc_debtor a
                WHERE a.account_code="1102050101.307" AND a.vstdate BETWEEN "'.$startdate.'" AND  "'.$enddate.'"
                order by a.vstdate desc;
            ');
    ...
    public function account_307_stam(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
                ...
                $check_an = Acc_1102050101_307::where('an', $value->an)->count();
                if ($check_an > 0) {
                    # code...
                    } else {
                        Acc_1102050101_307::insert([
