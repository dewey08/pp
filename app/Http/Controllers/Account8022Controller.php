...
class Account8022Controller extends Controller
 {
    ...
    public function account_pkti8022_search(Request $request)
    {
        ...
        if ($startdate =='') {
            $datashow = DB::select(
                'SELECT U1.* ,U2.pay_amount as Total_amount ,U1.STMdoc
                FROM acc_1102050102_8022 U1
             LEFT JOIN acc_stm_lgoti U2 on U2.hn = U1.hn AND U2.vstdate = U1.rxdate
    ...
    public function account_pkti8022_detail(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');
        $data['users'] = User::get();
        $data = DB::select('
        ...
        ');
        // GROUP BY U1.an
        return view('account_8022.account_pkti8022_detail', $data, [
            'data'       =>     $data,
            'months'     =>     $months,
            'year'       =>     $year
        ]);
