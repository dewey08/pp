...
use App\Models\Acc_1102050101_4022send;
...
class Account4022Controller extends Controller
 { 
    ...
    public function account_pkti4022_search(Request $request)
    {
        ...
        if ($startdate =='') {           
            $datashow = DB::select(
                'SELECT U1.sendactive,U1.acc_1102050101_4022_id,U1.an,U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.rxdate,U1.dchdate,U1.pttype,U1.debit_total,U2.Total_amount ,U2.STMdoc,U2.invno 
                from acc_1102050101_4022 U1
                LEFT JOIN acc_stm_ti_total U2 on U2.HDBill_hn = U1.hn AND U2.vstdate = U1.rxdate
    ...
    public function account_pkti4022_send(Request $request)
    {
        $id = $request->ids;
        $iduser = Auth::user()->id;
        $data = Acc_1102050101_4022::whereIn('acc_1102050101_4022_id',explode(",",$id))->get();
        Acc_1102050101_4022::whereIn('acc_1102050101_4022_id',explode(",",$id))
                    ->update([
                        'sendactive' => 'Y'
                    ]);
        foreach ($data as $key => $value) {
                $date = date('Y-m-d H:m:s');
             $check = Acc_1102050101_4022send::where('an', $value->an)->where('rxdate', $value->rxdate)->count();
                if ($check > 0) {
                    Acc_1102050101_4022send::where('an', $value->an)->where('rxdate', $value->rxdate)->update([
                        'hm'                => $value->hm,
                    ]);
                } else {
                    Acc_1102050101_4022send::insert([
                            'an'                => $value->an,
                            'vn'                => $value->vn,
                            'hn'                => $value->hn,
                            'cid'               => $value->cid,
