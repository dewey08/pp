...
class Account701Controller extends Controller
 {
    ...
    public function account_701_dash(Request $request)
    {
        ...
        // dd($startdate);
        return view('account_701.account_701_dash',$data,[
            'startdate'         =>  $startdate,
            'enddate'           =>  $enddate,
            'leave_month_year'  =>  $leave_month_year,
            'datashow'          =>  $datashow,
    ...
    //     return view('account_701.account_701_pull',[
    //         'startdate'     =>     $startdate,
    //         'enddate'       =>     $enddate,
    ...
    public function account_701_pull(Request $request)
    {
        ...
        $months        = date('m');
        $year          = date('Y');
        $newday        = date('Y-m-d', strtotime($datenow . ' -20 Day')); //ย้อนหลัง 1 สัปดาห์
        $startdate     = $request->startdate;
        $enddate       = $request->enddate;
        if ($startdate == '') {
            ...
            $acc_debtor = DB::select('
                ...
                WHERE a.account_code="1102050101.701" AND a.vstdate BETWEEN "' . $newday . '" AND "' . $datenow . '"
                group by a.vn
                order by a.vstdate asc
            ');
            // and month(a.dchdate) = "'.$months.'" and year(a.dchdate) = "'.$year.'"
            ...
            $acc_debtor = DB::select('
            SELECT * from acc_debtor a
            WHERE a.account_code="1102050101.701" AND a.vstdate BETWEEN "' . $startdate . '" AND "' . $enddate . '"
            group by a.vn
            order by a.vstdate asc
        ');
        }
    ...
    public function account_701_checksit(Request $request)
    {
    ...
    public function account_701_destroy(Request $request)
    {
        $id = $request->ids;
    ...
    public function account_701_search(Request $request)
    {
        $datenow       = date('Y-m-d');
        $months        = date('m');
        $year          = date('Y');
        ...
        if ($startdate == '') {
            // $acc_debtor = Acc_debtor::where('stamp','=','N')->whereBetween('dchdate', [$datenow, $datenow])->get();
            $datashow = DB::select('
                SELECT * from acc_1102050101_701 a
                WHERE a.vstdate BETWEEN "' . $newday . '" AND "' . $datenow . '"
                group by a.vn
                order by a.vstdate asc
            ');
            // and month(a.dchdate) = "'.$months.'" and year(a.dchdate) = "'.$year.'"
        ...
        return view('account_701.account_701_search',[
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate,
            'datashow'    =>     $datashow,
        ]);
    ...
    public function account_701_pulldata(Request $request)
    {
        $datenow = date('Y-m-d');
        ...
        $acc_debtor = DB::connection('mysql2')->select(
            'SELECT o.vn,o.an,o.hn,pt.cid,concat(pt.pname,pt.fname," ",pt.lname) ptname ,o.vstdate,o.vsttime
                ,v.hospmain,"" regdate,"" dchdate,op.income as income_group,ptt.pttype_eclaim_id,vp.pttype ,e.code as acc_code
                ,"1102050101.701" as account_code ,"บุคคลที่มีปัญหาสถานะและสิทธิ OP ใน CUP" as account_name ,v.income,v.uc_money,v.discount_money,v.paid_money,v.rcpt_money
                ...
                AND vp.pttype IN(SELECT pttype FROM pkbackoffice.acc_setpang_type WHERE pang ="1102050101.701")
        ...
        // $check = Acc_debtor::where('vn', $value->vn)->where('account_code','1102050101.701')->whereBetween('vstdate', [$startdate, $enddate])->count();
        foreach ($acc_debtor as $key => $value) {
                    $check = Acc_debtor::where('vn', $value->vn)->where('account_code','1102050101.701')->count();
                    if ($check == 0) {
                        Acc_debtor::insert([
                            'hn'                 => $value->hn,
                            'an'                 => $value->an,
    ...
    public function account_701_stam(Request $request)
    {
        $id = $request->ids;
        $iduser = Auth::user()->id;
        $data = Acc_debtor::whereIn('acc_debtor_id',explode(",",$id))->get();
        ...
        foreach ($data as $key => $value) {
             ...
             $check = Acc_1102050101_701::where('vn', $value->vn)->count();
    ...
    public function account_701_detail(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        ...
        ');
        // WHERE month(U1.vstdate) = "'.$months.'" and year(U1.vstdate) = "'.$year.'"
        return view('account_701.account_701_detail', $data, [
            'data'          =>     $data,
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate
        ]);
    }
    public function account_701_stm(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');
        $data['users'] = User::get();
        ...
        return view('account_701.account_701_stm', $data, [
            'datashow'      =>     $datashow,
            'months'        =>     $months,
            'year'          =>     $year
        ]);
    }
    public function account_701_stmnull(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');
        ...
        return view('account_701.account_701_stmnull',[
            'datashow'          =>     $datashow,
            'months'        =>     $months,
            'year'          =>     $year
        ]);
    ...
    public function account_701_detail_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        ...
        ');
        // WHERE month(U1.vstdate) = "'.$months.'" and year(U1.vstdate) = "'.$year.'"
        return view('account_701.account_701_detail_date', $data, [
            'data'          =>     $data,
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate
        ]);
    }
    public function account_701_stm_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');
        ...
        return view('account_701.account_701_stm_date', $data, [
            'datashow'         =>     $datashow,
            'startdate'        =>     $startdate,
            'enddate'          =>     $enddate
        ]);
    }
    public function account_701_stmnull_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');
        ...
        return view('account_701.account_701_stmnull_date',[
            'datashow'         =>     $datashow,
            'startdate'        =>     $startdate,
            'enddate'          =>     $enddate
        ]);
