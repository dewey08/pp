...
class AipnController extends Controller
{
    ...
    public function aipn_export(Request $request)
    {
        ...
        $file = new Filesystem;
        $file->cleanDirectory('C:Export'); //ทั้งหมด
        ...
        mkdir('C:Export/' . $folder, 0777, true);  //Web
        mkdir('C:Export/' . $foldertxt, 0777, true);  //Web
        ...
        // $file_pat = "Export_aipn/" . $foldertxt . "/10978-AIPN-" . $an . '-' . $aipn_date_now_preg . '' . $aipn_time_now_preg . ".txt";
            $file_pat = "C:Export/" . $foldertxt . "/10978-AIPN-670006147-". $aipn_date_now_preg . '' . $aipn_time_now_preg . ".txt";
    ...
    { 
        ...
        $iduser = Auth::user()->id;
         foreach ($data_aipn as $key => $v_main) {
             
            $aipn_data = DB::connection('mysql2')->select(  
                    'SELECT i.an,i.an as AN,i.hn as HN,"0" as IDTYPE,pt.cid as PIDPAT,pt.pname as TITLE,concat(pt.fname," ",pt.lname) as NAMEPAT 
                    ,pt.birthday as DOB,a.sex as SEX,pt.marrystatus as MARRIAGE,pt.chwpart as CHANGWAT,pt.amppart as AMPHUR,pt.citizenship as NATION
                    ,"C" as AdmType,"O" as AdmSource,i.regdate as DTAdm_d,i.regtime as DTAdm_t,i.dchdate as DTDisch_d,i.dchtime as DTDisch_t 
                    ,"0" AS LeaveDay,i.dchstts as DischStat,i.dchtype as DishType,"" as AdmWt,i.ward as DishWard,sp.nhso_code as Dept
                    ,ptt.hipdata_code maininscl,i.pttype,concat(i.pttype,":",ptt.name) pttypename ,"10702" HMAIN,"IP" as ServiceType
                    from ipt i
                    LEFT OUTER JOIN patient pt on pt.hn=i.hn
                    LEFT OUTER JOIN ptcardno pc on pc.hn=pt.hn and pc.cardtype="02"
                    LEFT OUTER JOIN an_stat a on a.an=i.an
                    ...
                    LEFT OUTER JOIN income inc on inc.income=oo.income
                    LEFT OUTER JOIN s_drugitems d on d.icode=oo.icode 
                    WHERE i.an = "' . $v_main->an . '"                   
                    AND ptt.pttype IN("A7")
                    group by i.an     
            ');
            // AND ptt.pttype IN("A7","s7","14")
            foreach ($aipn_data as $key => $value) {
            ...
            $aipn_billitems = DB::connection('mysql2')->select(
                ...
                ,oo.icode as LCCode,concat_ws("",d.name,d.strength) Descript,sum(oo.qty) as QTY,oo.UnitPrice as pricehos,dd.UnitPrice as pricecat
                ,sum(oo.sum_price) ChargeAmt_ ,dd.tmt_tmlt ,inc.income                
                ,case 
                when oo.paidst in ("01","03") then "T"
                else "D" end ClaimCat
                ...
                left outer join income inc on inc.income=oo.income                
                left outer join s_drugitems d on oo.icode=d.icode
                left join pkbackoffice.aipn_drugcat_labcat dd on dd.icode=oo.icode	
                left join pkbackoffice.aipn_labcat_sks ls on ls.lccode=oo.icode
                left join pkbackoffice.aipn_drugcat_sks dks on dks.hospdcode=oo.icode
            ...
            $aipn_ipop = DB::connection('mysql2')->select(
                'SELECT i.an as AN,"" as sequence,"ICD9CM" as CodeSys 
                    ,cc.icd9 as Code,icdname(cc.icd9) as Procterm,doctorlicense(cc.doctor) as DR                        
                    ,date_format(if(opdate is null,caldatetime(regdate,regtime),caldatetime(opdate,optime)),"%Y-%m-%dT%T") as DateIn
                    ,date_format(if(enddate is null,caldatetime(regdate,regtime),caldatetime(enddate,endtime)),"%Y-%m-%dT%T") as DateOut
                    ," " as Location
            ...
            $aipn_ipdx = DB::connection('mysql2')->select(
                'SELECT i.an as AN ,"" as sequence ,diagtype as DxType ,if(ifnull(aa.codeset,"")="TT","ICD-10-TM","ICD-10") as CodeSys
                    ,dx.icd10 as Dcode ,icdname(dx.icd10) as DiagTerm ,doctorlicense(cc.doctor) as DR ,null datediag
                    FROM ipt i
                    JOIN iptdiag dx on dx.an=i.an
                    JOIN iptoprt cc on cc.an=i.an
                    LEFT JOIN icd101 aa on aa.code=dx.icd10
    ...
    public function aipn_export_an___(Request $request)
    {
        $va1 = D_aipn_main::where('d_aipn_main_id','=','1')->first();
        $an = $va1->an;
        ...
        $datamain = DB::connection('mysql')->select('SELECT an FROM d_aipn_main');
            ...
            $opd_head = "\n" . '<IPDx Reccount="' . $ipdx_count . '">';
            fwrite($objFopen_opd, $opd_head);
            $ipdx = DB::connection('mysql')->select('SELECT * FROM d_aipdx');
            foreach ($ipdx as $key => $value_ip) {
                $s1 = $value_ip->sequence;
                $s2 = $value_ip->DxType;
                $s3 = $value_ip->CodeSys;
            ...
            $ipop = DB::connection('mysql')->select('SELECT sequence,CodeSys,Code,Procterm,DR,DateIn,DateOut,Location FROM d_aipop');
            foreach ($ipop as $key => $value_ipop) {
                $s1 = $value_ipop->sequence;
                $s2 = $value_ipop->CodeSys;
                $s3 = $value_ipop->Code;
