...
class Account702Controller extends Controller
 {
    ...
    public function account_702_dash(Request $request)
    {
        ...
        // dd($startdate);
        return view('account_702.account_702_dash',$data,[
            'startdate'         =>  $startdate,
            'enddate'           =>  $enddate,
            'leave_month_year'  =>  $leave_month_year,
            'datashow'          =>  $datashow,
    ...
    //                     SELECT a.acc_debtor_id,a.vn,a.an,a.hn,a.cid,a.ptname,a.vstdate,a.pttype,a.debit_total,a.subinscl,a.hospmain
    ...
    public function account_702_pull(Request $request)
    {
        ...
        if ($startdate == '') {
            ...
            $acc_debtor = DB::select('
                ...
                WHERE a.account_code="1102050101.702" AND a.vstdate BETWEEN "' . $newday . '" AND "' . $datenow . '"
                group by a.vn
                order by a.vstdate asc
            ');
            // and month(a.dchdate) = "'.$months.'" and year(a.dchdate) = "'.$year.'"
            ...
            $acc_debtor = DB::select('
            SELECT * from acc_debtor a
            WHERE a.account_code="1102050101.702" AND a.vstdate BETWEEN "' . $startdate . '" AND "' . $enddate . '"
            group by a.vn
            order by a.vstdate asc
        ');
        }
    ...
    public function account_702_pulldata(Request $request)
    {
        ...
        $acc_debtor = DB::connection('mysql2')->select(
            'SELECT o.vn,o.an,o.hn,pt.cid,concat(pt.pname,pt.fname," ",pt.lname) ptname ,o.vstdate,o.vsttime
                ...
                ,"1102050101.702" as account_code,"บุคคลที่มีปัญหาสถานะและสิทธิ OP นอก CUP" as account_name,v.income,v.uc_money,v.discount_money,v.paid_money,v.rcpt_money
    ...
    public function account_702_checksit(Request $request)
    {
    ...
    public function account_702_destroy(Request $request)
    {
        $id = $request->ids;
        $data = Acc_debtor::whereIn('acc_debtor_id',explode(",",$id))->get();
    ...
    public function account_702_search(Request $request)
    {
        $datenow       = date('Y-m-d');
        $months        = date('m');
        $year          = date('Y');
        ...
        if ($startdate == '') {
            // $acc_debtor = Acc_debtor::where('stamp','=','N')->whereBetween('dchdate', [$datenow, $datenow])->get();
            $datashow = DB::select('
                SELECT * from acc_1102050101_702 a
        ...
        return view('account_702.account_702_search',[
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate,
            'datashow'    =>     $datashow,
        ]);
