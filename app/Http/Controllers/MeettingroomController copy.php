...
{   
    ...
    $data['meeting_status_code'] = $request->query('meeting_status_code');
    ...
    if ($data['meeting_status_code'] != null) {
        $query = Meeting_service::select('meeting_service.*','meeting_status.*')
            ...
            // if($data['year'])
            // $query->where('meeting_service.meetting_year',$data['year']);
          
            // if($data['start'])
            // $query->where('meeting_service.meeting_date_begin',$data['start']);
            // $query->where('meeting_service.meeting_date_begin','>=',$data['start']);
            // $query->where('meeting_service.meeting_date_begin','=',$data['start']);
        
            // if($data['end'])
            // $query->where('meeting_service.meeting_date_begin',$data['end']);
            // $query->where('meeting_service.meeting_date_begin','<=',$data['end']);
        ...
        $query = Meeting_service::select('meeting_service.*','meeting_status.*')
            ...
            // if($data['meeting_status_code'])
            // $query->where('meeting_status.meeting_status_code',$data['meeting_status_code']);
